...
function load_day() {
    ...
    for (hour_block = settings.start; hour_block < settings.end; hour_block++) {
        ...
        row.setAttribute("id", hour_block);
        ...
        row.innerHTML = `<div class="col-1-xs hour">${time_format(hour_block)}</div>
                        ...
                        <div class="col-1-xs dragBtn"><div class="row icons"><i class="fas fa-bars"></i></div></div>
                        <div class="col-1-xs saveBtn"><div class="row icons"><i class="fas fa-save"></i></div></div>`;    
        ...
        document.querySelectorAll(".dragBtn").forEach(function (btn) {
            btn.addEventListener('mousedown', function (event) {
                for (let i = 0; i < event.path.length; i++) {
                    parent_element = event.path[i];
                    if (parent_element.classList.contains("hour_block")) {
                        parent_element.setAttribute('draggable','true');
                        break;
                    }
                }
            });
            ...
            btn.addEventListener('mouseout', function (event) {
                for (let i = 0; i < event.path.length; i++) {
                    parent_element = event.path[i];
                    if (parent_element.classList.contains("hour_block")) {
                        parent_element.removeAttribute('draggable');
                        break;
                    }
                }
            });
            ...
            btn.addEventListener('touchstart', function (event) {
                ...
                for (let i = 0; i < event.path.length; i++) {
                    ...
                    if (parent_element.classList.contains("hour_block")) {
                        touch_start = true;
                        parent_element.style.opacity = 0.33;
                        target_element = parent_element;
                        target_data = parent_element.querySelector(".appointment").value;
                        break;
                    }
                }
            });
...
function save_row(event) {
    ...
    // Cycles through each parent of the clicked button
    for (let i = 0; i < event.path.length; i++) {
        parent_element = event.path[i];

        // Checks for the correct parent and stores the data and exits the loop
        if (parent_element.classList.contains("hour_block")) {
            ...
            hour_save = parent_element.getAttribute("id");
            break;
        }
    }

    // Checks if the propagation missed the textarea
    if (text_save === null) {
        return;
    }
